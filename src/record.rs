...
impl RecordHeader {
    pub fn new(record_type: RecordType, flags: u32, form_id: Option<NonZeroU32>, size_of_subrecords: u32) -> Self {
        RecordHeader { record_type, flags, form_id, size_of_subrecords }
    }
...
impl Record {
    ...
    pub fn write<W: io::Write>(&self, game_id: GameId, writer: &mut W) -> io::Result<()> {
        ...
        if game_id != GameId::Morrowind {
            writer.write_all(&self.header.form_id.map(|x| x.get()).unwrap_or(0).to_le_bytes())?;
